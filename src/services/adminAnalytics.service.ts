...
export type DimKey = "location" | "category" | "priceRange" | "buyerAgeBucket" | "listingSize";
...
interface ListResponse<T> { data: T[]; pagination: { currentPage: number; lastPage: number; total: number } }

export const adminAnalyticsService = {
  get: () =>
    apiClient.get<ItemResponse<AdminAnalytics>>("/api/v1/admin/analytics"),

  marketingLeads: (params: { page?: number; size?: number; search?: string; leadStatus?: string } = {}) => {
    ...
    return apiClient.get<ListResponse<AdminMarketingLead>>(
      `/api/v1/admin/analytics/marketing-leads${query ? `?${query}` : ""}`,
    );
  },
};