  ...
  return useQuery({
    ...
    queryFn: async (): Promise<UserPreferences | null> => {
      ...
      const { data, error } = await supabase
        ...
        .select("styles, brands, preferred_fit, budget_min, budget_max, onboarding_completed")
        .eq("user_id", user.id)
        .maybeSingle();
 ...
 */
export const personalizeListings = <T extends { brand: string; category: string; price: number }>(
  listings: T[],
  ...
  const scored = listings.map(item => {
    ...
    if (prefs.budget_min != null && prefs.budget_max != null) {
      if (item.price >= prefs.budget_min && item.price <= prefs.budget_max) score += 1;
    }
    return { item, score };
  });
