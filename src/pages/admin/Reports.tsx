...
const getTargetType = (
  r: AdminReport,
): "listing" | "user" | "message" | "conversation" | null => {
...
const Reports = () => {
  ...
  return (
    <div className="space-y-6">
      ...
      <Tabs
        value={filter}
        onValueChange={(v) => setFilter(v as ReportStatus | "all")}
      >
        <TabsList>
            ...
            return (
              <Card key={r.id}>
                <CardContent className="space-y-3 p-4">
                  ...
                  {r.status !== "OPEN" && (
                    <p className="text-xs text-muted-foreground">
                      Resolved by {r.resolverFullName ?? "—"} ·{" "}
                      {r.resolvedAt
                  ...
                  <Textarea
                    ...
                    value={notes}
                    onChange={(e) =>
                      setNotesDraft({ ...notesDraft, [r.id]: e.target.value })
                    }
                    rows={2}
                  />
